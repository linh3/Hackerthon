...
namespace DungeonsandDragons
{
    public class Battle
    {
        ...
        public Character usePotion()
        {
            if (HeroesTurn)
            {
                Heroes[HeroTurnNumber].Health = Heroes[HeroTurnNumber].MaxHealth;
                potion--;
                HeroTurnNumber++;
            }
            else
            {
                Monsters[MonsterTurnNumber].Health = Monsters[MonsterTurnNumber].MaxHealth;
                potion--;
                MonsterTurnNumber++;
            }
        ...
        public Character takeTurn(bool usePotion = false)
        {

            if (HeroesTurn)
            {
                Monster Defender = (Monster)whoDefenseNext();
                if (usePotion && potion > 0)
                {
                    Heroes[HeroTurnNumber].Health = Heroes[HeroTurnNumber].MaxHealth;
                    potion--;
                }
        ...
        {
            this.HeroTurnNumber = this.HeroTurnNumber % NUMPLAYER;
            this.MonsterTurnNumber = this.MonsterTurnNumber % NUMPLAYER;
            if(HeroesTurn)
            {
